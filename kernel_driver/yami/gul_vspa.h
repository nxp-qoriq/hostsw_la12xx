...
#define VSPA_FW_NAME_PREFIX			"geul-vspa"
#define VSPA_WARMUP_FW_NAME_PREFIX	"geul-vspa-warmup"

/* Overlay */
#define GUL_PEBM_OVERLAY_BASE	0xE0380000
#define GUL_PEBM_BASE_ADDR	0xE0200000
#define GUL_OVERLAY_CORE_SIZE	0x10000	// 1 Overlay Section size (HOST OR PEBM)
#define GUL_PEBM_OVERLAY_OFFSET	0x180000 //Last 512 KB memory

/* FECA HRAM */
#define GUL_FECA_BASE_ADDR		0xE5000000
...
#define VSPA_INSTANCE_OFFSET 0x4000
#define GUL_VSPA_SIZE 0x10000

#define VSPA_DCSR_OFFSET 0x48000
#define VSPA_DBG_INSTANCE_OFFSET 0x2000
...
#define PRAM_ADDR_MASK  0x0007FFFF
#define VSPA_DRAM_SIZE  0x00050000	/*320KB (128K-VDRAM+192K-IDRAM) */
#define DMA_BCNT_MASK   0x0001FFFF
...
#define ERR(...) { if (vspadev->debug & DEBUG_MESSAGES) \
		dev_err(vspadev->dev, VSPA_DEVICE_NAME __VA_ARGS__); }
...
#define VCPU_GO_ADDR_REG_OFFSET         (0x0180>>2)
...
/* Number of memspace in VSPA */
#define MAX_VSPA_MEMSPACE	4
/* Maximum size of memspace name */
#define MAX_MEMSPACE_NAME_SZ	16

typedef struct {
	uint32_t memspace_id;
	uint32_t flags; /* rwx */
	uint32_t start_address;
	uint32_t size;
	uint32_t memspace_namesz; /* size of memspace name including the final terminator ('\0') */
	char	 memspace_name[MAX_MEMSPACE_NAME_SZ];
} mw_info_type_14;

#define MW_INFO_TYPE_14		(14)
...
#define MW_INFO_TYPE_15		(15)

struct vspa_section_info {
	uint32_t vspa_sec_count;
	mw_info_type_14 memspace_map[MAX_VSPA_MEMSPACE];
	uint32_t vspa_memspace_count;
	mw_info_type_15 *section_memspace_map;
};
...
struct vspa_device {
	...
	/*Dma region for vspa from gul_dev*/
	struct gul_mem_region_info vspa_dma_region;
	struct gul_mem_region_info *overlay_region;
...
/* TODO	spinlock_t irq_lock; */
	...
	struct vspa_section_info vspa_sec_info;
};
