...
#
# Usage: scripts/la12xx_memory_calc.py -i ../geul_freertos_la12xx/demos/E200_MPC57XXX_GEUL_GCC/release/geul_e200.map -o geul_e200_memory.dump
...
#globals
lmc_version = "1.0"
lmc_elf_filename = "release/geul_e200.map"
lmc_output_filename = "release/geul_e200_memory.dump"
test = False
search_specific_mode = False
specific_symbol_name = ""

usage = "Usage: \n \
              -i <path>/geul_e200.map : Input mapfile to check, if not given \"release/geul_e200.map\" is taken by default \n \
...
END_STRING = "_end ="
PREFIX_STRING = "__"
POWERPC_LIBRARY_STRING = "powerpc-eabivle"
FILL_STRING = "*fill*"
FREERTOS_STRING = "geul_freertos_la12xx/"
...
class LA12xx_Memory_Calc:
    ...
    def process_section_mem_config(self, section_name, line, prev_line):
        ...
        if line.find(PREFIX_STRING + section_name + START_STRING) != -1:
            return
        if line.find(FILL_STRING) != -1:
            return
        values = line.split()

        if len(values) == 4: #case 1: .smem          0xe01f0030     0x1458 CMakeFiles/geul.dir/opt/samba/nxa19332/platform/GEUL/Bonnyrigg/geul_freertos_la12xx/lib/ipi/ipiQueue.c.obj
            if values[3].find(CMAKEFILES_STRING) == -1 and values[3].find(POWERPC_LIBRARY_STRING) == -1:
                return
            ...
            if values[3].find(CMAKEFILES_STRING) != -1:
                        ...
                        secconfig_map_item[SYMBOL_FILE_NAME_STRING] = values[3]
        ...
        elif len(values) == 3: # case 3: 0xe0a40000       0x90 CMakeFiles/geul.dir/opt/samba/nxa19332/platform/GEUL/Bonnyrigg/geul_freertos_la12xx/utils/Test-Framework/tc_core_smem_text.c.obj
            if values[2].find(CMAKEFILES_STRING) == -1 and values[2].find(POWERPC_LIBRARY_STRING) == -1:
                return
            secconfig_map_item = {SYMBOL_NAME_STRING: "Unknown", SYMBOL_START_ADDR_STRING: 0, SYMBOL_LEN_STRING: 0, SYMBOL_FILE_NAME_STRING: "Unknown"}
            ...
            if values[2].find(CMAKEFILES_STRING) != -1:
                        ...
                        secconfig_map_item[SYMBOL_FILE_NAME_STRING] = values[2]
            try:
            ...
            if symbol_len > 0:
                ...
                self.sectionconfig_map.append(secconfig_map_item)
                self.sectionconfig_count += 1
        elif len(values) == 2: # case 2: 0xe01f0030                IPIGlobalQueueMem
            ...
            if self.sectionconfig_count >= 1:
                if self.sectionconfig_map[self.sectionconfig_count - 1][SYMBOL_NAME_STRING] == "Unknown" \
                    or self.sectionconfig_map[self.sectionconfig_count - 1][SYMBOL_NAME_STRING].find(section_name) != -1 \
    ...
    def process_elf_mem_config(self, line):
        ...
        if line.find(NAME_STRING) != -1:
            return
        values = line.split()
        memconfig_map_item = {NAME_STRING: values[0], START_ADDR_STRING: values[1], RSV_END_ADDR_STRING: hex(int(values[1], 16) + int(values[2], 16)), END_ADDR_STRING: "NA", AVAILABLE_KB_LEN_STRING: "NA", USED_KB_LEN_STRING: "NA", FREE_KB_LEN_STRING: "NA"}
